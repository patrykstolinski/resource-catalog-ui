...
const FeedbackForm = ( { resourceId, onFeedbackSubmitted } ) => {
    ...
    return(
        <form onSubmit={handleSubmit} className="space-y-4">
            { successMessage && (
                <div className="bg-green-50 border-l-4 border-green-400 text-green-800 p-4 rounded-r-xl">
                    <p className="font-bold">Erfolg!</p>
                    <p>{successMessage}</p>
                </div>
            )}
            <textarea className="w-full p-3 border border-gray-300 rounded-lg transition focus:outline-none"
                rows="4"
            ...
            <button className="bg-main-dark text-white py-2 px-6 rounded-lg hover:bg-main-secondary transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-semibold"
                type="submit"
